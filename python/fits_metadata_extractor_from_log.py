...
def extract_metadata_from_log(log_file, output_file=None, delete_json=False):
    ...
    if output_file is None:
        ...
        milliseconds = timestamp_now.microsecond // 1000
        tsv_timestamp = f"{timestamp_now.strftime('%Y%m%d_%H%M%S')}_{milliseconds:03d}"
        output_file = f'fits_metadata_{tsv_timestamp}.tsv'
    
    print(f"Output file: {output_file}")
    print("Writing incrementally (safe for interruption)\n")
    
    # Extract metadata from first file to establish column structure
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Analyzing first file for column structure...")
    
    first_row = copied_df.iloc[0]
    first_filepath = first_row['destination_file']
    first_metadata = extract_fits_metadata(first_filepath)
    
    # Get column ordering from utils
    stats_keys = get_stats_column_names()
    
    # Add organizer log columns we want to include
    # target from log, target_from_path will come from extract_fits_metadata
    log_columns = ['target', 'filter', 'exposure_sec', 'gain', 'temperature_c', 
                   'temp_folder', 'timestamp']
    
    # Get header keys from first file
    header_keys = sorted([k for k in first_metadata.keys() 
                         if k not in stats_keys and k not in log_columns])
    
    # Column order: stats (includes filepath, target_from_path) + log columns + headers
    ordered_keys = stats_keys + log_columns + header_keys
    
    # Open output file and write header
    with open(output_file, 'w') as f:
        f.write('\t'.join(ordered_keys) + '\n')
    
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Column structure established ({len(ordered_keys)} columns)")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Processing files...\n")
    
    # Process files and append incrementally
    successful = 0
    failed = 0
    start_time = datetime.now()
    ...
    with open(output_file, 'a') as f:
        for i, (idx, row) in enumerate(copied_df.iterrows(), 1):
            ...
            if metadata is not None and 'extraction_error' not in metadata:
                # Combine FITS metadata with organizer log data
